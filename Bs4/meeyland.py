...
def crawlDataFirstTime(start, end):
    ...
    file_path = PATH_FILE_LOG + "/" + "meeyland.csv"
    ...
    for i  in range  (start,end):
        ...
        try:
            ...
            Csv.write_log(PATH_FILE_LOG, "meeyland-" + datetime.datetime.now().strftime('%Y%m%d'),
                          'meeyland -' + str(err) + '-' + url+'/page-'+str(i))
            continue
...
def crawlBySchedule():
    ...
    #make folder store CSV files
    Path("/tmp/leveldb/meeyland").mkdir(parents=True, exist_ok=True)
    file_path = PATH_FILE_LOG + "/" + "meeyland-" + datetime.datetime.now().strftime('%Y%m%d') + ".csv"
    writeFieldNameToFile(file_path)
    iterator = 0
    while stop == 0:
        page = page + 1
        ...
        try:
            ...
            Csv.write_log(PATH_FILE_LOG, "meeyland-" + datetime.datetime.now().strftime('%Y%m%d'),
                          'meeyland -' + str(err) + '-' + url + '?page=' + str(page))
            continue
        list_link = soup.find_all('div', class_='col-sm-6 col-md-4')
        for link in list_link:
            d = {}
...
if first_time == '1':
    ...
    writeFieldNameToFile(PATH_FILE_LOG + "/" + "meeyland.csv")
    final = getFinalPage('https://meeyland.com/mua-ban-nha-dat/ho-chi-minh')
